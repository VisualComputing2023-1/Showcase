...
class Quadrille {
    ...
    static OP(quadrille1, quadrille2, operator, row = 0, col = 0) {
      ...
      let quadrille = new Quadrille(
        col < 0
          ? Math.max(quadrille2.width, quadrille1.width - col)
          : Math.max(quadrille1.width, quadrille2.width + col),
        row < 0
          ? Math.max(quadrille2.height, quadrille1.height - row)
          : Math.max(quadrille1.height, quadrille2.height + row)
      );
    ...
    constructor() {
      if (arguments.length === 1) {
        ...
        if (Array.isArray(arguments[0])) {
          ...
          let memory2D = arguments[0].map((array) => {
            return array.slice();
          });
          let width;
      ...
      if (
        ...
        this._memory2D = Array(arguments[1])
          .fill()
          .map(() => Array(arguments[0]).fill(0));
        return;
      }
      if (
        arguments.length === 2 &&
        ...
        this._memory2D = Array(
          Math.round((arguments[0] * arguments[1].height) / arguments[1].width)
        )
          .fill()
          .map(() => Array(arguments[0]).fill(0));
      ...
      if (
        ...
        this._memory2D = Array(
          Math.ceil(arguments[1].toString(2).length / arguments[0])
        )
          .fill()
          .map(() => Array(arguments[0]).fill(0));
    ...
    _format(memory1D, size) {
      for (let i = 0; i < memory1D.length; i++) {
        if (
          typeof memory1D[i] !== "number" &&
          typeof memory1D[i] !== "string" &&
          !Array.isArray(memory1D[i]) &&
          !(memory1D[i] instanceof p5.Color) &&
          !(memory1D[i] instanceof p5.Image)
        ) {
          memory1D[i] = 0;
        }
      }
    ...
    from() {
      if (arguments.length === 1 && arguments[0] instanceof p5.Image) {
        ...
        image.loadPixels();
        let r = Array(this.height)
          .fill()
          .map(() => Array(this.width).fill(0));
        let g = Array(this.height)
          .fill()
          .map(() => Array(this.width).fill(0));
        let b = Array(this.height)
          .fill()
          .map(() => Array(this.width).fill(0));
      ...
      if (arguments.length === 2 && typeof arguments[0] === "number") {
        ...
        for (let i = 0; i <= length - 1; i++) {
          if (bitboard & (1 << (length - 1 - i))) {
            this._memory2D[this._fromIndex(i).row][this._fromIndex(i).col] =
              arguments[1];
          }
        }
      }
    ...
    fill() {
      ...
      if (
        ...
        if (
          arguments[0] >= 0 &&
          arguments[0] < this.height &&
          arguments[1] >= 0 &&
          arguments[1] < this.width
        ) {
    ...
    _conv(mask, row, col, cache_half_size = (mask.width - 1) / 2) {
      if (
        row >= cache_half_size &&
        col >= cache_half_size &&
        row < this.height - cache_half_size &&
        col < this.width - cache_half_size
      ) {
        let r = 0;
        let g = 0;
        let b = 0;
        for (let imask = 0; imask < mask.height; imask++) {
          for (let jmask = 0; jmask < mask.width; jmask++) {
            ...
            if (
              (neighbour instanceof p5.Color || Array.isArray(neighbour)) &&
              typeof mask_value !== "string" &&
              !(mask_value instanceof p5.Image)
            ) {
              // luma coefficients are: 0.299, 0.587, 0.114, 0
              let weight =
                typeof mask_value === "number"
                  ? mask_value
                  : 0.299 * red(mask_value) +
                    0.587 * green(mask_value) +
                    0.114 * blue(mask_value);
        ...
        for (let i = 0; i < this.height; i++) {
          for (let j = 0; j < this.width; j++) {
            ...
            if (coords.w0 >= 0 && coords.w1 >= 0 && coords.w2 >= 0) {
              let r =
                red(pattern0) * coords.w0 +
                red(pattern1) * coords.w1 +
                red(pattern2) * coords.w2;
              let g =
                green(pattern0) * coords.w0 +
                green(pattern1) * coords.w1 +
                green(pattern2) * coords.w2;
              let b =
                blue(pattern0) * coords.w0 +
                blue(pattern1) * coords.w1 +
                blue(pattern2) * coords.w2;
              let a =
                alpha(pattern0) * coords.w0 +
                alpha(pattern1) * coords.w1 +
                alpha(pattern2) * coords.w2;
    ...
    _barycentric_coords(row, col, row0, col0, row1, col1, row2, col2) {
      let edges = this._edge_functions(
        row,
        col,
        row0,
        col0,
    ...
    transpose() {
      ...
      this._memory2D = this._memory2D[0].map((_, i) =>
        this._memory2D.map((row) => row[i])
      );
    }
    ...
    rotate() {
      ...
      this._memory2D = this._memory2D[0].map((_, i) =>
        this._memory2D.map((row) => row[i]).reverse()
      );
    }
    ...
    sort({
      ...
      switch (mode) {
        case "DISTANCE":
          memory1D.sort((cellA, cellB) => {
            ...
            let wa = Math.sqrt(
              Math.pow(sa.r / sa.total - red(target), 2) +
                Math.pow(sa.g / sa.total - green(target), 2) +
                Math.pow(sa.b / sa.total - blue(target), 2) +
                Math.pow(sa.a / sa.total - alpha(target), 2)
            );
            let wb = Math.sqrt(
              Math.pow(sb.r / sb.total - red(target), 2) +
                Math.pow(sb.g / sb.total - green(target), 2) +
                Math.pow(sb.b / sb.total - blue(target), 2) +
                Math.pow(sb.a / sb.total - alpha(target), 2)
            );
            return wa - wb;
          });
          break;
      ...
      this._init1D(
        (memory1D = ascending ? memory1D : memory1D.reverse()),
        this.width
      );
    }
  ...
  (function () {
    ...
    p5.prototype.drawQuadrille = function (
      quadrille,
      {
        graphics = this,
        pixelX = 0,
      ...
      graphics.push();
      graphics.translate(
        pixelX > 0 ? pixelX : col * cellLength,
        pixelY > 0 ? pixelY : row * cellLength
      );
      for (let i = 0; i < quadrille._memory2D.length; i++) {
        for (let j = 0; j < quadrille._memory2D[i].length; j++) {
          graphics.push();
